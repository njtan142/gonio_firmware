...
static void peer_connection_state_new(PeerConnection* pc, DtlsSrtpRole role, int isOfferer) {
  ...
  ret = dtls_srtp_init(&pc->dtls_srtp, role, pc);
  if (ret != 0) {
    LOGE("dtls_srtp_init failed -0x%.4x", (unsigned int)-ret);
    return;
  }
...
int peer_connection_loop(PeerConnection* pc) {
  ...
  switch (pc->state) {
    ...
    case PEER_CONNECTION_COMPLETED:
      ...
      // Deferred from CONNECTED state to avoid a race condition:
      // dtls_srtp_write() inside sctp_create_association() can trigger the
      // BIO recv callback, which polls agent_recv() while state == CONNECTED.
      // That consumes Chrome's SCTP INIT-ACK before this COMPLETED loop can
      // route it to sctp_incoming_data(), causing ~60s exponential backoff.
      // By deferring to COMPLETED, the BIO recv loop exits immediately
      // (state != CONNECTED), and the INIT-ACK is read on the next iteration.
      if (pc->config.datachannel && pc->sctp.dtls_srtp == NULL) {
        LOGI("SCTP create association (deferred to COMPLETED state)");
      ...
      if (data) {
        if (pc->config.datachannel == DATA_CHANNEL_STRING)
          sctp_outgoing_data(&pc->sctp, (char*)data, bytes, PPID_STRING, pc->sctp.data_sid);
        else
          sctp_outgoing_data(&pc->sctp, (char*)data, bytes, PPID_BINARY, pc->sctp.data_sid);
        buffer_pop_head(pc->data_rb);
      }
#endif

      if ((pc->agent_ret = agent_recv(&pc->agent, pc->agent_buf, sizeof(pc->agent_buf))) > 0) {
#if ENABLE_PC_DATA_TRACE
        LOGI("agent_recv %d bytes (type=0x%02x)", pc->agent_ret, pc->agent_buf[0]);
#endif
...
#if ENABLE_PC_DATA_TRACE
          LOGI("dtls_srtp_read returned %d", ret);
#endif
...
void peer_connection_set_remote_description(PeerConnection* pc, const char* sdp_text) {
  ...
  while ((line = strstr(start, "\r\n"))) {
    ...
    if (strstr(buf, "a=setup:passive") || strstr(buf, "a=setup:actpass")) {
  ...
  if (!pc->b_local_description_created) {
    LOGI("DTLS role selected: %s", role == DTLS_SRTP_ROLE_CLIENT ? "client(active)" : "server(passive)");
