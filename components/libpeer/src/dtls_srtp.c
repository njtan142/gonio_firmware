...
int dtls_srtp_init(DtlsSrtp* dtls_srtp, DtlsSrtpRole role, void* user_data) {
  ...
  if (ret < 0) {
    LOGE("dtls_srtp_selfsign_cert failed");
    return ret;
  }
...
static int dtls_srtp_do_handshake(DtlsSrtp* dtls_srtp) {
  ...
  LOGI("dtls_do_handshake: starting");
  do {
    ret = mbedtls_ssl_handshake(&dtls_srtp->ssl);
    iter++;
    if (ret != 0) {
      LOGI("dtls_do_handshake: iter=%d ret=-0x%04x",
           iter, (unsigned int)-ret);
    }
  } while (ret == MBEDTLS_ERR_SSL_WANT_READ || ret == MBEDTLS_ERR_SSL_WANT_WRITE);

  LOGI("dtls_do_handshake: finished after %d iterations, ret=-0x%04x", iter, (unsigned int)-ret);
  return ret;
}
...
static int dtls_srtp_handshake_server(DtlsSrtp* dtls_srtp) {
  ...
  LOGI("dtls_handshake_server: starting");
  while (1) {
    unsigned char client_ip[] = "test";
    round++;

    if (need_reset) {
      LOGI("dtls_handshake_server: round=%d session reset", round);
    ...
    LOGI("dtls_handshake_server: round=%d calling do_handshake", round);
    ret = dtls_srtp_do_handshake(dtls_srtp);

    if (ret == MBEDTLS_ERR_SSL_HELLO_VERIFY_REQUIRED) {
      LOGI("dtls_handshake_server: round=%d HELLO_VERIFY_REQUIRED", round);
      need_reset = 1;

    } else if (ret != 0) {
      LOGE("dtls_handshake_server: round=%d failed ret=-0x%.4x", round, (unsigned int)-ret);
      break;

    } else {
      LOGI("dtls_handshake_server: round=%d success", round);
      break;
    }
  }

  LOGI("dtls_handshake_server: done after %d rounds, ret=-0x%04x", round, (unsigned int)-ret);
  return ret;
}
...
int dtls_srtp_write(DtlsSrtp* dtls_srtp, const unsigned char* buf, size_t len) {
  ...
  do {
    ...
    if (ret == MBEDTLS_ERR_SSL_WANT_READ) {
      LOGI("dtls_srtp_write: WANT_READ (BIO recv will poll agent)");
    }
  } while (ret == MBEDTLS_ERR_SSL_WANT_READ || ret == MBEDTLS_ERR_SSL_WANT_WRITE);
  return ret;
}
