  ...
  while ((4 * (pos + 3) / 4) < len) {
    ...
    switch (chunk_common->type) {
      case SCTP_DATA: {
        ...
        LOGI("SCTP_DATA ppid=%lu sid=%u data[0]=0x%02x", (unsigned long)ntohl(data_chunk->ppid), ntohs(data_chunk->sid), data_chunk->data[0]);
...
// XXX: unordered sequence
      ...
      case SCTP_COOKIE_ECHO: {
        LOGI("SCTP_COOKIE_ECHO received");
      ...
      case SCTP_COOKIE_ACK: {
        LOGI("SCTP_COOKIE_ACK received — association established");
        break;
      }
      case SCTP_ABORT:
        sctp->connected = 0;
